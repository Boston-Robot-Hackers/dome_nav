...
#   - implement next_goal(ctx) -> (x, y) | None
#       return a world-frame goal, or None to mean "nothing to send".
#
...
# goal to completion before asking again. Returning None long enough trips the
# node's NO_FRONTIER_PATIENCE and ends exploration.
#
...
class HelloWorldAlgorithm:
    ...
    def next_goal(
        ...
        # Already sent our single goal -> nothing more to do. The node reads this
        # None as "no target"; after NO_FRONTIER_PATIENCE ticks it declares done.
        if self.emitted:
            return None
        self.emitted = True
